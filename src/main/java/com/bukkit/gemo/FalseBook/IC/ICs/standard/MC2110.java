...
public class MC2110 extends BaseIC
{
  ...
  public MC2110() {
    ...
    this.ICDescription = "The MC2110 delays a low signal for [x] seconds (Line 3), if the signal is stable.";
  }
  ...
  public void Execute(Sign signBlock, InputState currentInputs, InputState previousInputs)
  {
    ...
    {
      for (newSched = this.TaskList.values().iterator(); ((Iterator)newSched).hasNext(); ) { SchedulerClass sched = (SchedulerClass)((Iterator)newSched).next();
        if (sched.equalsLoc(signBlock.getBlock().getLocation())) {
          Bukkit.getServer().getScheduler().cancelTask(sched.TaskID);
          this.TaskList.remove(signBlock.getBlock().getLocation().toString());
          break;
  ...
  {
    public int TaskID = -1;
    public MC2110 father;
    public Location signLoc;

    public SchedulerClass(MC2110 father, Location signLoc)
    {
      this.father = father;
